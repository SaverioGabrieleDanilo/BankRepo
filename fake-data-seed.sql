...
INSERT INTO user_statuses (name) VALUES ('ACTIVE'), ('SUSPENDED'), ('CLOSED')
    ON CONFLICT (name) DO NOTHING;

INSERT INTO registration_statuses (name) VALUES ('PENDING'), ('APPROVED'), ('REJECTED')
    ON CONFLICT (name) DO NOTHING;

INSERT INTO account_statuses (name) VALUES ('ACTIVE'), ('PENDING'), ('REJECTED'), ('CLOSED')
    ON CONFLICT (name) DO NOTHING;

INSERT INTO transaction_type (name) VALUES ('DEPOSIT'), ('WITHDRAWAL'), ('BANK_TRANSFER'), ('INTERNAL_TRANSFER')
    ON CONFLICT (name) DO NOTHING;

INSERT INTO transaction_status (name) VALUES ('EXECUTED')
    ON CONFLICT (name) DO NOTHING;
...
VALUES
    ('Elena', 'Ferrari', '1990-03-15', 'elena.ferrari@example.com', 'employee1',
        (SELECT id FROM roles WHERE name = 'EMPLOYEE'), (SELECT id FROM user_statuses WHERE name = 'ACTIVE'),
...
VALUES
    ...
    -- Registrazione ancora da approvare: nessun conto associato. Stato utente
    -- SUSPENDED coerente con registration_status PENDING (vedi UserServiceImpl:
    -- un utente non ancora approvato non puo' essere ACTIVE).
    ('Luca', 'Bianchi', '1995-11-03', 'luca.bianchi@example.com', 'lbianchi',
        NULL,
        (SELECT id FROM roles WHERE name = 'CUSTOMER'), (SELECT id FROM user_statuses WHERE name = 'SUSPENDED'),
    ...
    -- Registrazione ancora da approvare: nessun conto associato (vedi nota su lbianchi)
...
VALUES
    ('IT32X0542811101000000100001', (SELECT id FROM users WHERE username = 'mrossi'),
        544.00, 544.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '60 days', now() - interval '60 days', now() - interval '1 days'),

    ('IT05X0542811101000000100002', (SELECT id FROM users WHERE username = 'mrossi'),
        500.00, 500.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '40 days', now() - interval '40 days', now() - interval '6 days'),

    ('IT75X0542811101000000100003', (SELECT id FROM users WHERE username = 'gverdi'),
        1100.00, 1100.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '45 days', now() - interval '45 days', now() - interval '1 days'),

    ('IT48X0542811101000000100004', (SELECT id FROM users WHERE username = 'aneri'),
        0.00, 0.00, (SELECT id FROM account_statuses WHERE name = 'CLOSED'),
        now() - interval '120 days', now() - interval '120 days', now() - interval '10 days'),

    ('IT21X0542811101000000100005', (SELECT id FROM users WHERE username = 'fcolombo'),
        892.00, 892.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '55 days', now() - interval '55 days', now() - interval '8 days'),

    ('IT91X0542811101000000100006', (SELECT id FROM users WHERE username = 'dgalli'),
        0.00, 0.00, (SELECT id FROM account_statuses WHERE name = 'PENDING'),
        now() - interval '20 days', now() - interval '20 days', now() - interval '20 days'),

    ('IT64X0542811101000000100007', (SELECT id FROM users WHERE username = 'schiara'),
        950.00, 950.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '33 days', now() - interval '33 days', now() - interval '3 days'),

    ('IT37X0542811101000000100008', (SELECT id FROM users WHERE username = 'mesposito'),
        1490.00, 1490.00, (SELECT id FROM account_statuses WHERE name = 'ACTIVE'),
        now() - interval '70 days', now() - interval '70 days', now() - interval '12 days'),

    ('IT10X0542811101000000100009', (SELECT id FROM users WHERE username = 'lferri'),
        0.00, 0.00, (SELECT id FROM account_statuses WHERE name = 'PENDING'),
...
VALUES
    ((SELECT id FROM transaction_type WHERE name = 'DEPOSIT'), (SELECT id FROM transaction_status WHERE name = 'EXECUTED'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT37X0542811101000000100008'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT37X0542811101000000100008'),
    ...
    ((SELECT id FROM transaction_type WHERE name = 'WITHDRAWAL'), (SELECT id FROM transaction_status WHERE name = 'EXECUTED'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT21X0542811101000000100005'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT21X0542811101000000100005'),
        (SELECT id FROM users WHERE username = 'fcolombo'), (SELECT id FROM users WHERE username = 'fcolombo'),
        200.00, 'Prelievo sportello', now() - interval '15 days', now() - interval '15 days', now() - interval '15 days'),

    ((SELECT id FROM transaction_type WHERE name = 'BANK_TRANSFER'), (SELECT id FROM transaction_status WHERE name = 'EXECUTED'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT37X0542811101000000100008'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT32X0542811101000000100001'),
    ...
    ((SELECT id FROM transaction_type WHERE name = 'INTERNAL_TRANSFER'), (SELECT id FROM transaction_status WHERE name = 'EXECUTED'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT32X0542811101000000100001'),
        (SELECT id FROM bank_accounts WHERE iban = 'IT05X0542811101000000100002'),
        (SELECT id FROM users WHERE username = 'mrossi'), (SELECT id FROM users WHERE username = 'mrossi'),
...
-- ---------------------------------------------------------------------
-- Rubrica beneficiari (solo per i customer con conto ATTIVO). Alcuni IBAN
-- coincidono con conti gia' seminati sopra (cosi' i bonifici verso questi
-- contatti finiscono su conti reali), altri sono IBAN esterni fittizi
-- (banca/CAB diversi da quelli usati per i conti interni) per simulare
-- beneficiari fuori dal gestionale.
-- ---------------------------------------------------------------------
INSERT INTO contacts (user_id, name, surname, iban, email, note, created_at, updated_at)
VALUES
    ((SELECT id FROM users WHERE username = 'mrossi'), 'Giulia', 'Verdi',
        'IT75X0542811101000000100003', 'giulia.verdi@example.com', 'Affitto mensile', now() - interval '40 days', now() - interval '40 days'),
    ((SELECT id FROM users WHERE username = 'mrossi'), 'Matteo', 'Esposito',
        'IT37X0542811101000000100008', 'matteo.esposito@example.com', 'Spese condominiali comuni', now() - interval '35 days', now() - interval '35 days'),
    ((SELECT id FROM users WHERE username = 'mrossi'), 'Enel Energia', 'S.p.A.',
        'IT60Y0306901600000012345001', NULL, 'Bolletta luce e gas', now() - interval '20 days', now() - interval '20 days'),

    ((SELECT id FROM users WHERE username = 'gverdi'), 'Mario', 'Rossi',
        'IT32X0542811101000000100001', 'mario.rossi@example.com', 'Coinquilino, quota affitto', now() - interval '38 days', now() - interval '38 days'),
    ((SELECT id FROM users WHERE username = 'gverdi'), 'Credem', 'Amministrazione Immobiliare',
        'IT79W0301522200000055667004', 'amministrazione@credem-immobili.example.com', 'Padrone di casa', now() - interval '30 days', now() - interval '30 days'),

    ((SELECT id FROM users WHERE username = 'fcolombo'), 'Sara', 'Chiari',
        'IT64X0542811101000000100007', 'sara.chiari@example.com', 'Quota affitto condiviso', now() - interval '25 days', now() - interval '25 days'),
    ((SELECT id FROM users WHERE username = 'fcolombo'), 'FitClub', 'Palestra',
        'IT54Z0200812345000098765002', NULL, 'Abbonamento mensile palestra', now() - interval '18 days', now() - interval '18 days'),

    ((SELECT id FROM users WHERE username = 'schiara'), 'Francesca', 'Colombo',
        'IT21X0542811101000000100005', 'francesca.colombo@example.com', 'Quota affitto condiviso', now() - interval '25 days', now() - interval '25 days'),
    ((SELECT id FROM users WHERE username = 'schiara'), 'Amministratore', 'Condominio Via Roma',
        'IT73J0100544100000033445005', NULL, 'Spese condominiali', now() - interval '15 days', now() - interval '15 days'),

    ((SELECT id FROM users WHERE username = 'mesposito'), 'Mario', 'Rossi',
        'IT32X0542811101000000100001', 'mario.rossi@example.com', 'Rimborso spese comuni', now() - interval '13 days', now() - interval '13 days'),
    ((SELECT id FROM users WHERE username = 'mesposito'), 'Assicurazioni Generali', 'S.p.A.',
        'IT39K0538776543000011223003', NULL, 'Polizza RC auto', now() - interval '10 days', now() - interval '10 days')
